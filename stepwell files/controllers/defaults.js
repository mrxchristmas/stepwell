// const domain = 'http://development.prodocuflow.com/';
// const skidbuilderDomain = 'http://skidbuilder.prodocuflow.com/';
const domain = 'http://localhost:8080/stepwell/';
const skidbuilderDomain = 'http://localhost:8080/skidbuilder_ceis/';
let databaseid;
...
$('.modules-widget').click(function(){
    ...
    console.log("clicked on: ", mod);
    if(mod.includes('skidbuilder')){
        const obj ={
            '__ID' : __ID,
            '__USER_LEVEL' : __USER_LEVEL,
            '__PASSWORD' : __PASSWORD,
            '__PHOTO' : __PHOTO,
            '__FIRST_NAME' : __FIRST_NAME,
            '__COMPANY_ID' : __COMPANY_ID,
            '__COMPANY_NAME' : __COMPANY_NAME,
            '__COMPANY_LOGO' : __COMPANY_LOGO,
            '__DATABASE_ID' : __DATABASE_ID
        }
        ...
        window.location.href = skidbuilderDomain + '?function=redirect&' + ZURL;
    }else{
        window.location.href = domain + 'pages/' + mod;
    }
});
