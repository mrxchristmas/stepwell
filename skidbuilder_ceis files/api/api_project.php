...
if (isset($_GET['function']) && $_GET['function']!="") {
    ...
    if($_GET['function'] == 'fetchAlert'){
        $ownerid = $_GET['ownerid'];

        $query = "SELECT * FROM tbl_alert WHERE ownerid = '$ownerid';";
        $rs = mysqli_query($link , $query);
        if(mysqli_num_rows($rs)>0){
            $x = 0;
            while($row = $rs->fetch_assoc()) {
                $response[$x]['id'] = $row['id'];
                $response[$x]['ownerid'] = $row['ownerid'];
                $response[$x]['fn'] = $row['function'];
                $response[$x]['dataview'] = $row['dataview'];
                $response[$x]['dataapprove'] = $row['dataapprove'];
                $response[$x]['datareject'] = $row['datareject'];
    ...
    if($_GET['function'] == 'deleteAlert'){
        $id = $_GET['id'];

        $query = "DELETE FROM tbl_alert WHERE id = '$id';";
